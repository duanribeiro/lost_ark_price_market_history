...
import os
import logging
from text_extractors import window_data, earrings_helpers
...
def extract_data_from_image(client, mongo_collection):
    screenshot_folder = r'C:\pics\earrings'
    ...
    for index, filename in enumerate(os.listdir(screenshot_folder)):
        ...
        if index % 11 == 0:
            ...
            try:
                detected_item_texts = earrings_helpers.detect_text_amazon_rekognition(
                    client=client, image_path=file_path, folder_path=screenshot_folder
                )
                parsed_items = earrings_helpers.clean_detected_texts(texts=detected_item_texts)
